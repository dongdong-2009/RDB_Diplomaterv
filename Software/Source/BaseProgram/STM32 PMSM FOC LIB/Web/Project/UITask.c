...
bool UITask_SendBufferContentOnUsart(uint8_t *pBuffer, uint16_t sizeToSend)
{
  bool bResult = FALSE;
  static uint8_t *pErrorMessage = ((uint8_t *) "\n\r\n\r\n\r\n\rCommunication error: Overrun\n\r\n\r\n\r\n\r");
  
  if(TRUE == COM_IsBufferFree((CCOM)oUSART))
  {
    COM_SendBuffer((CCOM)oUSART, pBuffer, sizeToSend);
    bResult = TRUE;
  }
  else
  {
    COM_SendBuffer((CCOM)oUSART, pErrorMessage, strlen((const char *) pErrorMessage));
  }
